<?php
// We MUST include our config file to get the subject lists and connect to the DB.
require_once 'admin/config.php';

// Helper function to parse video URLs and get details
function get_video_details($url) {
    ...
    // Add enablejsapi=1 parameter for programmatic control
...
// Get the correct list of subjects for this year from our central config.
$subjects_for_this_year = isset(SUBJECTS_BY_YEAR[$year_to_show]) ? SUBJECTS_BY_YEAR[$year_to_show] : [];
?>
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <title>NU Questions (Year <?php echo $year_to_show; ?>) - Saadat Notebook</title>
...
<body>
    ...
    <div class="page-wrapper animate__animated animate__fadeIn">
        <main>
            <h1 class="page-title">NU Questions - Honours <?php echo $year_to_show; echo ($year_to_show == 1) ? 'st' : 'nd'; ?> Year</h1>

            <div class="subject-filter-container" id="subject-filter-container">
                <h3 class="subject-filter-label">Filter by subject</h3>
                <div class="filter-buttons-grid">
                    <button class="filter-btn active" data-subject="all">All Subjects</button>
                    <?php
                    foreach ($subjects_for_this_year as $subject) {
                        $safe_subject_check = $conn->real_escape_string($subject);
                        $sql_check = "SELECT id FROM uploads WHERE category = 'NU Questions' AND subject = '$safe_subject_check' AND academic_year = $year_to_show LIMIT 1";
                        $result_check = $conn->query($sql_check);
                        if ($result_check->num_rows > 0) {
                            $subject_slug = strtolower(preg_replace('/[^a-zA-Z0-9]+/', '-', $subject));
                            echo '<button class="filter-btn" data-subject="' . $subject_slug . '">' . htmlspecialchars($subject) . '</button>';
                        }
                    }
                    ?>
                </div>
            ...
            foreach ($subjects_for_this_year as $subject):
                ...
                $sql = "SELECT id, title, file_link, preview_link, description, video_links_json 
                        FROM uploads 
                        WHERE category = 'NU Questions' AND subject = '$safe_subject' AND academic_year = $year_to_show 
                        ORDER BY upload_date DESC";
                $result = $conn->query($sql);
                
                if ($result && $result->num_rows > 0):
                    $section_slug = strtolower(preg_replace('/[^a-zA-Z0-9]+/', '-', $subject));
            ?>
            
            <div class="subject-section" id="<?php echo $section_slug; ?>">
                <h2 class="subject-heading"><?php echo htmlspecialchars($subject); ?></h2>
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        const filterContainer = document.getElementById('subject-filter-container');
        if (filterContainer) {
            ...
            filterButtons.forEach(button => {
                button.addEventListener('click', () => {
                    const subjectToShow = button.dataset.subject;
                    filterButtons.forEach(btn => btn.classList.remove('active'));
                    button.classList.add('active');
                    
                    subjectSections.forEach(section => {
                        if (subjectToShow === 'all' || section.id === subjectToShow) {
